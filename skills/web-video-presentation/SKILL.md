...
| Phase 3 音频合成 | `references/AUDIO.md`（含 narrations.ts → segments.json → 任意 provider 流程，内置 minimax + openai） | `references/TIMELINE-PLAYBACK.md`（如果想跳过音频合成、直接用逐字稿时间线驱动自动播放） |
| Phase 4 录屏 + 后期 | `references/RECORDING.md`（含 `?auto=1` 自动录屏 + `?timeline=1` 无音频自动录屏） | —— |
...
- **双源原则**：节奏跟口播稿（顺序不能乱），细节回原文章抽（信息池 +
  本章 article 段落）
- **动画库**：复杂动画（多步编排、滚动驱动、SVG 变形/绘制、文字特效）推荐 **GSAP**。所有插件现已免费，按需加载对应 GSAP skill（gsap-core / gsap-timeline / gsap-scrolltrigger / gsap-react / gsap-plugins / gsap-performance / gsap-utils / gsap-frameworks）。简单动画仍优先 CSS transition/animation
...
要不要合成音频做"自动播放录屏"？
  ...
  ✗ 不合成但有逐字稿 → 用 Timeline 模式：node scripts/map-timeline.mjs
           生成 timeline.ts，然后用 ?timeline=1 一镜到底录屏。
           详见 references/TIMELINE-PLAYBACK.md。
  ✗ 都不做 → 跳过 Phase 3，直接 Phase 4 用手动录屏 + 后期配音。
```
...
| [`references/RECORDING.md`](references/RECORDING.md) | Phase 4 才读 | 录屏工具 + 后期合成（含 `?auto=1` + `?timeline=1` 一镜到底） |
| [`references/TIMELINE-PLAYBACK.md`](references/TIMELINE-PLAYBACK.md) | 有逐字稿想跳过音频合成时 | 基于 SRT 逐字稿时间线驱动自动播放，无需音频文件 |
...
| `../gsap-core/SKILL.md` 等 8 份 GSAP skill | 写章节动画 / 滚动驱动 / SVG / 文字特效时按需读 | GSAP 动画库全家桶：tween、timeline、ScrollTrigger、React、plugins、performance、utils、frameworks |
